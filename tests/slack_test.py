...
def test_get_slack_user_id_status_400_list(user_with_email: SlackOps, caplog):
    ...
    assert (
        "Get https://slack.com/api/users.list returned status code 400" in caplog.text
    )


def test_get_slack_user_id_ok_false_list(user_with_email: SlackOps, caplog):
    with requests_mock.Mocker() as m:
        m.get(
            "https://slack.com/api/users.list",
            status_code=200,
...
def test_get_slack_user_id_status_400_email(user_with_email: SlackOps, caplog):
    with requests_mock.Mocker() as m:
        m.get(
            "https://slack.com/api/users.list",
            status_code=200,
    ...
    assert (
        "Get https://slack.com/api/users.lookupByEmail returned status code 400"
        in caplog.text
    )


def test_get_slack_user_id_ok_false_email(user_with_email: SlackOps):
    with requests_mock.Mocker() as m:
        m.get(
            "https://slack.com/api/users.list",
            status_code=200,
        ...
        m.get(
            "https://slack.com/api/users.list",
            status_code=200,
            json={"ok": False, "error": "No users found"},
        )
