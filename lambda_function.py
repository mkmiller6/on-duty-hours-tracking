"""
Lambda function that is triggered by OpenPath events on clock-in and clock-out buttons.
"""
...
class OpenpathEvent:
    ...
    def __post_init__(self):
        self.timestamp_datetime = datetime.fromtimestamp(self.timestamp,
...
def handler(event, _):
    ...
    op_event = OpenpathEvent(op_event.get('entry'),
                              op_event.get('userId'),
    ...
    existing_sheet_check = drive_service.files().list( # pylint: disable=maybe-no-member
                                q=f'''
    ...
    if len(existing_sheet_check.get('files')) > 0:
        ...
        sheet_id = drive_service.files().copy( # pylint: disable=maybe-no-member
    ...
    if op_event.entry == "Instructors Locker":
        ...
        if not exists:
            ...
            new_sheet_id = new_sheet.get('replies')[0] \
            .get('addSheet') \
            .get('properties') \
            .get('sheetId')

            batch_update_new_sheet(sheet,
